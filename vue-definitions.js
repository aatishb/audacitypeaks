...
let app = new Vue({
  ...
  methods: {
    ...
    secondDerivative(a, i) {
      if (this.smoothing == 0) {
        ...
        return (
          (2 * a[i - 2] - a[i - 1] - 2 * a[i] - a[i + 1] + 2 * a[i + 2]) / 7
        );
      } else if (this.smoothing == 2) {
        return (
          (5 * a[i - 3] -
  ...
  computed: {
    spectrum() {
      let spectrumData = this.text
        .split(/\r\n|\n|\r/) // Split on both CRLF and LF
      ...
      for (i = 1; i < this.spectrum.freq.length; i++) {
        ...
        if (p1.y >= 0 && p2.y < 0) {
          ...
          if (this.secondDerivative(this.spectrum.amp, i) < -this.slopeCutoff) {
            let peakFreq =
              this.spectrum.amp[i] > this.spectrum.amp[i + 1] ? p1.x : p2.x;
            let peakAmp =
              this.spectrum.amp[i] > this.spectrum.amp[i + 1]
                ? this.spectrum.amp[i]
                : this.spectrum.amp[i + 1];
